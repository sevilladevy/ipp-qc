...
const SUPABASE_URL = process.env.VITE_SUPABASE_URL || "https://ohnvjdzonmzybfqryrau.supabase.co";
...
async function runMigration() {
  ...
  try {
    // Use pg endpoint to run raw SQL
    ...
    console.log("\nTrying alternative approach via Management API...");

    const response = await fetch(
      `https://api.supabase.com/v1/projects/ohnvjdzonmzybfqryrau/database/query`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      console.error("❌ Management API also failed");
    }
  }
}
