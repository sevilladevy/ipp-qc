// Execute migration directly using pg library
// Run: node scripts/exec-migration.mjs
...
if (!DB_URL) {
  ...
  console.log(
    "Get it from: https://supabase.com/dashboard/project/ohnvjdzonmzybfqryrau/settings/database",
  );
...
async function runMigration() {
  ...
  console.log("Database URL:", DB_URL.replace(/:[^:@]+@/, ":****@")); // Mask password
  ...
  try {
    ...
    // Verify
    const result = await client.query("SELECT COUNT(*) FROM public.defect_types");
    console.log(`Defect types count: ${result.rows[0].count}`);

    const result2 = await client.query(
      "SELECT COUNT(*) FROM public.inspection_table_default_parts",
    );
    console.log(`Table default parts count: ${result2.rows[0].count}`);
  } catch (error) {
